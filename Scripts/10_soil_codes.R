...
soil_freq_country <- 
  ggplot(soil_data_R, aes(fill = Country, x = soil)) +
  geom_bar(width = 0.5) +
  labs(title = "",
       x = "Soil Type",
       y = "Frequency") +
...
# Boxplots of MP concentrations in different soil types 
...
#-----------------------------------------------------------------------
# Importing data from HWSD

# DO NOT DO ANYTHING IN THIS SECTION UNLESS DATA SET IS UPDATED - OTHERWISE 
# YOU WILL HAVE TO MANUALLY ADD IN SOIL TYPES BACK IN AGAIN 
